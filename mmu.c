...
void allocate_memory(list_t * flist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    if (block_node->blk->end != blk_end){
      ...
      block_t *frag = (block_t*)malloc(sizeof(block_t));
      
      
      frag->pid = 0;
      
      
      frag->start = block_node->blk->end + 1;
      ...
      if (policy == 1){
        list_add_to_back(flist, frag);
      }
      else if (policy == 2){
        list_add_ascending_by_blocksize(flist, frag);
      }
      else if (policy == 3){
        list_add_descending_by_blocksize(flist, frag);
      }
    }
}

void deallocate_memory(list_t * alloclist, list_t * flist, int pid, int policy) { 
     /* if policy == 1 -> FIFO
     *              2 -> BESTFIT 
     *              3 -> WORSTFIT
     * 
    ...
    */
    node_t *currs = alloclist->head;
    node_t *prev = NULL;
    block_t *deallocate_blk = NULL;

    
    while (currs && currs->blk->pid != pid) {
      prev = currs;
      currs = currs->next;
    }
    ...
    if (policy == 1){
      list_add_to_back(flist, deallocate_blk);
    }
    else if (policy == 2){
      list_add_ascending_by_blocksize(flist, deallocate_blk);
    }
    else if (policy == 3){
      list_add_descending_by_blocksize(flist, deallocate_blk);
    }
}
